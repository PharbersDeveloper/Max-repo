...
map<-read.xlsx(paste0('L:/全原盘数据/D盘备份/Pharbers文件/医院/医院大全月度更新版本（持续更新中……）/',
...
map.cpa.pha<-read.xlsx(paste0('L:/全原盘数据/D盘备份/',
                              'Pharbers文件/医院/医院大全月度更新版本（持续更新中……）/',
                              '2019年Universe更新维护1.0_190403.xlsx'),
                       sheet='Mapping')
...
map_all<-map.cpa.pha[!is.na(map.cpa.pha$医院编码),c('新版ID','CPA')]%>%
...
CPA_old<-read.xlsx('E:/MAX/Tide/Global/190815/190814泰德-1701-1906检索.xlsx',
                    sheet='1701-1906数据')

CPA_new <- read_xls('E:/MAX/Tide/UPDATE/1908/191014泰德-8月检索.xls',
                    sheet='8月数据')
table(CPA_new$年,CPA_new$月,useNA='always')

CPA <- rbind(CPA_new,CPA_old[CPA_old$月 %in% CPA_new$月,])

table(CPA$年,CPA$月,useNA='always')

CPA$医院编码 <- as.numeric(CPA$医院编码)

total.rawdata <- CPA%>%data.frame(stringsAsFactors = F)%>%
...
sum(total.rawdata$Sales[is.na(total.rawdata$PHA.ID)])/
...
####注意，这个增长率文件并不包含所有的原始ID，只包含历史年份文件在universe的ID。
####月度更新补数的时候匹不到的用城市再看看
growth.rate.with.id <- read.xlsx('E:/MAX/Tide/MODEL/凯纷/000ref/各个分子分城市等级1718增长率文件.xlsx')
...
raw.data.complete <- raw.data.add[raw.data.add$Year %in% substr(c_month,1,4),]


names(raw.data.complete)
...
##补数前后的原始部分两个值要相等
sum(total.rawdata$Sales[total.rawdata$Year %in% substr(c_month,1,4)])
...
##补数前后的原始部分两个值要相等
sum(total.rawdata$Units[total.rawdata$Year %in% substr(c_month,1,4)])
...
write.csv(raw.data.complete,
          paste0('E:/MAX/Tide/UPDATE/',substr(c_month,3,6),'/',c_month,'补数结果.csv'),
          row.names = F)
