...
universe <- read.xlsx("E:/MAX/Tide/MODEL/凯那/000ref/universe.xlsx")
...
factorize <- function(mkt, y1) {
  ...
  max_detail1 <-
    fread(
      paste(
        'L:/MAX result/Tide/',y1,
        '/MAX_',mkt,'_20000_wo_ot_',y1,'.csv',
        sep = ""
      ),
      stringsAsFactors = F,
      header = T
  ...
  panel15 <-
    read.xlsx(paste0('E:/MAX/Tide/UPDATE/',y1,'/',mkt,'_Panel_',y1,'.xlsx'))
